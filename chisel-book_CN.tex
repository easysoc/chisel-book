...
\ifbook
\else
\includepdf{chisel-cover-CN.pdf}
\newpage
\thispagestyle{empty}
~
\newpage
...
\ \\
\vspace{1cm}
{\usekomafont{title}\Huge Chisel 数字电路设计\\
\bigskip
{\usekomafont{title}\huge 第二版}\\
\bigskip
\bigskip
\bigskip
\bigskip
...
目前，数字电路设计是一个令人兴奋的领域。随着 Dennard 缩放比例定律的终结和摩尔定律（Moore's Law）的放缓，对于该领域的创新需求似乎从未如此迫切。半导体公司仍然在竭尽所能的从硬件上榨取每一滴性能，但其相关改进的成本已急剧增加。Chisel 可以通过提高设计效率来降低总的成本花销。如果设计人员可以在更短的时间内完成更多的工作，同时通过复用现有实现来摊销验证成本，则公司可以在一次性工程费用（Non-Recurring Engineering）上花费更少的钱；此外，学生和个人开发者也可以借此更轻松地进行创新工作。
...
Chisel 不同于其它语言的地方在于它是嵌入到另一个编程语言 Scala 中的领域专用语言（Domain Specified Language 或 DSL）。本质上，Chisel 是一个用来描述和呈现数字电路中基本元素的类和函数库。基于 Chisel 的电路设计其实就是编写 Scala 程序，在执行的过程中\emph{产生} 对应的电路。很多人会产生疑问：``为什么 Chisel不能像 VHDL 或 SystemVerilog 那样作为一门独立的语言？''。我的回答是：在过去的几十年中，软件领域已经在设计方法论方面进行了大量的创新，与其尝试使用这些技术去创建新的硬件语言，不如简单地使用现代编程语言，从而能够直接利用这些软件领域的创新工作。
...
一个长期存在的对于 Chisel 的批评是它``难以学习''。这种感觉很大程度上因为这些初学者选择了过于复杂的入门示例，而这些大型、复杂的设计往往由专家为解决自己的研究或商业需求而创建。当学习一种比较流行的语言如 C++ 时，我们不是从阅读 GCC 编译器的源代码开始。相反，有许多课程，教科书和其他学习材料来满足这些入门人员。Martin 通过本书为想学习 Chisel 的人提供了一个重要的资源。
...
Martin 是一位经验丰富的教育家，这在本书的组织结构中得到体现。从安装和基础组件开始，他通过类似构建一栋建筑一样的方式，一步一步地使读者理解所需内容。每章末尾随附的练习是巩固理解的重要补充，可以确保每个概念都牢牢扎根于读者的脑海。书中重要的一章\emph{硬件生成器}贯通了本书所介绍的基本内容。最终，以读者能够掌握构建一个简单而有用的 RISC 处理器结束本书。
...
Martin 通过本书，为读者能够高效的进行数字设计奠定了坚实的基础。进而，读者可以通过这些基础，构建任何他想要的内容。
...
本书主要内容为使用硬件构建语言 Chisel 进行数字电路设计。Chisel将软件工程中的面向对象和函数式等高级语言特性引入数字电路设计。本书的读者对象为硬件设计人员和软件工程师。具有 Verilog 和 VHDL 背景的硬件设计师可以通过使用更现代的语言来提高他们的工作效率；而具有面向对象和函数式编程经验的软件工程师可以利用他们所掌握的知识对硬件进行编程，例如在云端运行的 FPGA 加速器。

本书通过介绍典型中小规模的硬件电路组件来探索基于 Chisel 的数字电路设计。
...
类似 Chisel 允许通过敏捷方法设计硬件，开放访问和按需印刷使得通过敏捷的方式发布教科书成为可能。因此，在本书第一版发布后不到6个月的时间内，作者可以提供改进和修订之后的第二版。

除了少量的问题修复，第二版的主要变化如下：\\
扩充测试章节内容；时序逻辑电路构建模块一章包含更多的示例；一个新的章节用来介绍同步输入信号的处理，内容包括消除电路中的抖动和对输入信号中的噪声进行过滤。扩充设计示例一章来介绍不同的 FIFO 设计实现，这些 FIFO 变种也展示了在进行数字设计时如何使用类型参数和继承等特性。
...
感谢所有 Chisel 的开发者，感谢他们创建了如此酷的硬件构建语言。Chisel 使用起来非常愉快，因此值得为此写一本书。
感谢整个 Chisel 社区，它热情，友好而乐于回答有关 Chisel 的任何问题。

我还要感谢最近几年中，学习高级计算机体系结构课程的我的学生，他们中的大多数人都选择了 Chisel 作为毕业项目。感谢你们避开舒适的环境，而选择学习和使用最新的硬件描述语言，你们的很多问题帮助塑造了本书。
...
本书主要介绍使用现代硬件构建语言 \myref{https://www.chisel-lang.org/}{Chisel}~\cite{chisel:dac2012} 进行数字系统设计。在本书中，我们主要关注于在一个较高级别的抽象层次和短时间内，设计相对复杂、可交互的数字电路系统。
...
本书读者对象为两组不同的开发者：(1) 硬件设计师 和 (2) 软件工程师，擅长 VHDL 或者 Verilog 语言的硬件设计人员，他们往往也使用 Python，Java 或者 Tcl 来生成硬件，这些设计人员可以迁移到一个单一的硬件构建语言，而硬件生成功能是这门语言自身的一部分。软件工程师或许也开始对硬件设计感兴趣，例如未来的 Intel 芯片将会包含可编程结构来加速执行程序。将 Chisel 作为首次使用的硬件描述语言是一个非常不错的选择。
...
Chisel 将软件工程中的面向对象和函数式等高级语言特性引入数字设计。Chisel 不仅允许在寄存器传输级描述硬件电路，而且也允许读者编写硬件生成器。
...
目前，硬件电路通常使用硬件描述语言来构建。使用 CAD 工具绘制硬件组件的时代早就成为过去，一些高级别的原理图可以用来概述整个系统，但不会用来描述系统实现。两种经常使用的硬件描述语言分别是 Verilog 和 VHDL。两种语言都比较古老，具有许多过时的特性，并且对于语言的哪些结构能够综合为硬件具有不断变化的标准。然而，不要误解，VHDL 和 Verilog 确实都能够很好的描述可综合到 \myref{https://en.wikipedia.org/wiki/Application-specific_integrated_circuit}{ASIC} 的硬件。使用 Chisel 进行硬件设计时，Verilog 作为一种中间语言存在，用来进行测试和综合。
...
本书不是一本专门用来介绍电路设计基础的书，如果你想寻找有关数字电路基础的内容，例如，如何通过 CMOS 晶体管构建门电路，请参考其它数字电路设计书籍。本书主要目的在于教授在一个抽象层次上以 ASIC 或 \myref{https://en.wikipedia.org/wiki/Field-programmable_gate_array}{FPGA} 为目标的数字电路设计。
\footnote{相对于 ASIC，作者对 FPGA 更为熟悉些，本书中呈现的一些设计优化主要面向 FPGA 工艺。}
阅读本书的前置条件是，我们假设读者具有基本的 \myref{https://en.wikipedia.org/wiki/Boolean_algebra}{布尔代数} 和 \myref{https://en.wikipedia.org/wiki/Binary_number}{二进制系统} 的知识，当然，还需要一些基本的编程经验（任何语言均可）。本书不需要读者熟悉 Verilog 或 VHDL，Chisel 可以做为你的第一种数字硬件描述语言。由于本书示例代码的构建基于\code{sbt} 和 \code{make}，因此，基本的命令行（也叫做终端或Unix shell）使用经验将会很有帮助。
...
Chisel 自身并不是一种很庞大的语言，其基本的结构可以用\myref{https://github.com/freechipsproject/chisel-cheatsheet/releases/latest/download/chisel_cheatsheet.pdf}{一页}内容描述，并且可以在几天内掌握。因此，本书也不会是一本很复杂的书。Chisel 的内容要比 VHDL 和 Verilog 少的多。Chisel 真正强大的地方在于其嵌入在一个富有表现力的 \myref{https://www.scala-lang.org/}{Scala} 语言之中。Chisel 继承了 Scala ``和你一同成长''~\cite{Scala} 的语言特性。不过，Scala 并不是本书的主题，我们提供了一个较短的章节来向硬件设计人员介绍 Scala。由 Odersky ~\cite{Scala} 等人编写的参考书对 Scala 进行了详细的介绍。另外，本书是一本数字电路设计和Chisel语言特性的基本指南，不是有关 Chisel 的 API 参考手册，也不是一本详细描述芯片设计的书。
...
本书中所有的代码示例提取自可编译和测试的完整程序，因此，代码应该不会包含语法错误。代码示例存储在本书的 \myref{https://github.com/schoeberl/chisel-book}{GitHub 仓库}。除了展示代码本身，我们也想向读者展示一些有用的设计和良好的硬件描述风格。

本书针对笔记本和平板电脑（如 iPad）进行了优化，我们在一些文本中包含了辅助阅读内容的链接，这些内容大多指向\myref{https://en.wikipedia.org/}{维基百科}中的文章。

\section{安装 Chisel 工具}
...
基本上，每本介绍编程语言的书都会有一个叫作 \emph{Hello World} 的最小示例，以下即遵循该范式的代码：
...
然而，这是 Chisel 代码吗？或者一个用来打印字符串的硬件电路？并不是，它仅仅是纯粹的 Scala 代码，还不足以表示硬件设计中的 ``Hello World''。
...
这里列出了数字电路设计和有关 Chisel 的一些辅助阅读材料：
\begin{itemize}
...
\item
\myref{http://www.cambridge.org/es/academic/subjects/engineering/circuits-and-systems/digital-design-systems-approach}{Digital Design: A Systems Approach} 是一本较新的关于数字电路设计的的教材，它有两个版本：分别使用 Verilog 或 VHDL 作为硬件描述语言。
...
\item \myref{https://www.chisel-lang.org/}{Chisel} 官方主页是下载和学习 Chisel 的起点.
\item \myref{https://github.com/ucb-bar/chisel-tutorial}{Chisel Tutorial} 提供包含测试和解决方案的练习项目

\item \myref{https://github.com/freechipsproject/chisel3/wiki}{Chisel Wiki} 包含有关 Chisel 的用户指南和更多内容链接
\item \myref{https://github.com/freechipsproject/chisel-testers}{Chisel Testers} 在其代码仓库中可以找到详细介绍的 Wiki 文档
\item \myref{https://github.com/freechipsproject/chisel-bootcamp}{Generator Bootcamp} 以\myref{https://jupyter.org/}{Jupyter}笔记形式展示基于 Chisel 的硬件生成器设计
\item \myref{https://github.com/schoeberl/chisel-lab}{chisel-lab} 包含一些丹麦技术大学``Digital Electronics 2''课程的 Chisel 练习

\end{itemize}

\footnote{译者注：为避免 Chisel 初学者困惑，本书在翻译时移除了和 FPGA 有关的内容和练习，如有需要请参考原文}
%\section{练习}
...
\subsection{通过 Pull Request 贡献代码}
